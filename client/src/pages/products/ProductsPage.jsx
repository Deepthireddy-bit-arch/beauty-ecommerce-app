...
// import { selectBrands, fetchBrands } from '../../redux/slices/brandpageSlice';
...
function ActiveFilterChips({
  ...
  selectedDiscounts.forEach((d) => {
    chips.push({
      key: `disc-${d}`,
      label: `${d}% off`,
      remove: () => setSelectedDiscounts(selectedDiscounts.filter((x) => x !== d)),
    });
  });
...
export default function ProductsPage() {
  ...
  // ── Read URL search params (?category=makeup, ?search=...) ──
  ...
  const [sidebarOpen,       setSidebarOpen]       = useState(false);
  const [isFirstLoad,       setIsFirstLoad]       = useState(true);
  // Track whether we've already seeded from URL params so we don't loop
  const [urlSeeded,         setUrlSeeded]         = useState(false);

  /* ══════════════════════════════════════════════════════════
     STEP 1 — Seed Redux from URL params on first mount only.
     Runs once; subsequent filter changes update URL (step 2).
  ══════════════════════════════════════════════════════════ */
  useEffect(() => {
    if (urlSeeded) return;
  ...
  /* ══════════════════════════════════════════════════════════
     STEP 2 — Keep URL in sync whenever filters change.
     This makes links shareable and back-button aware.
  ══════════════════════════════════════════════════════════ */
  useEffect(() => {
    if (!urlSeeded) return; // don't run before seed completes
  ...
  }, [selectedCategory, searchQuery, sortBy, selectedBrands, page, urlSeeded]); // eslint-disable-line react-hooks/exhaustive-deps

  /* ══════════════════════════════════════════════════════════
     STEP 3 — Fetch products whenever any filter/sort/page
     changes. Depends on Redux state (already set in step 1).
  ══════════════════════════════════════════════════════════ */
  useEffect(() => {
    if (!urlSeeded) return; // wait for seed so we don't fire with stale 'All'
    ...
    sortBy,
    page,
    selectedBrands.join(','),   // eslint-disable-line react-hooks/exhaustive-deps
    minPrice,
    maxPrice,
    selectedDiscounts.join(','), // eslint-disable-line react-hooks/exhaustive-deps
  ]);

  /* ── Bootstrap: brands + wishlist ── */
  useEffect(() => {
    dispatch(fetchBrands());
    dispatch(fetchWishlist());
  }, [dispatch]);
  ...
  /* ── Track first load ── */
  useEffect(() => {
    if (!loading && products.length > 0) setIsFirstLoad(false);
  }, [loading, products]);
    ...
    <div className="pp-root">
      ...
      <div className="pp-body">
        ...
        <aside className={`pp-sidebar${sidebarOpen ? ' pp-sidebar--open' : ''}`}>
          ...
          <div className="filter-panel-header">
            <div className="filter-panel-title">
              <span>Filters</span>
              {activeFilterCount > 0 && (
                <span className="filter-total-badge">{activeFilterCount}</span>
              )}
            </div>
            {activeFilterCount > 0 && (
              <button className="clear-filters-link" onClick={clearAll}>Clear all</button>
            )}
          </div>
            ...
            {DISC_OPTIONS.map((d) => (
              <FilterCheckbox
                ...
                onChange={() => {
                  setSelectedDiscounts(
                    selectedDiscounts.includes(d.value)
                      ? selectedDiscounts.filter((x) => x !== d.value)
                      : [...selectedDiscounts, d.value]
                  );
                  dispatch(setPage(1));
                }}
          ...
          <button className="pp-sidebar-apply" onClick={() => setSidebarOpen(false)}>
            Apply Filters ({activeFilterCount} active)
          </button>
        </aside>
        ...
        <div className="pp-main">
          ...
          {!showSkeletons && !error && products.length > 0 && (
            <p className="pp-results-count">
              ...
              {selectedCategory && selectedCategory !== 'All' && (
                <span className="pp-results-cat"> in <strong>{selectedCategory}</strong></span>
              )}
            </p>
          )}
          ...
          {!showSkeletons && !error && products.length === 0 && (
            <div className="pp-state pp-state--nodata">
              <img src={noDataImg} alt="No products found" className="pp-nodata-img" />
              <h5>No products found</h5>
              <p>Try adjusting your filters or search term.</p>
              <button className="pp-retry-btn" onClick={clearAll}>Clear Filters</button>
            </div>
          )}
